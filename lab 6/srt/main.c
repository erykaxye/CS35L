...
int nthreads; 
scene_t scene; 
float threads[width][height][3]; 

void *init_arr(void *thread_num) 
{
  Vec3 camera_pos;
  ...
  const double pixel_dy = tan( 0.5*camera_fov ) / ((double)height*0.5);
    ...
    int thread_count = *(int*) thread_num; 
    for( int px=thread_count; px<width; px+=nthreads )
      {
        ...
        for( int py=0; py<height; ++py )
	  {
	    ...
	    */
	    ...
	    threads[px][py][0] = scaled_color[0];
            threads[px][py][1] = scaled_color[1];
            threads[px][py][2] = scaled_color[2];
	  }
      }
    return NULL;
}
...
main( int argc, char **argv )
{
    ...
    for(i = 0; i < nthreads; i++)
      {
	thread_num[i] = i;
	err = pthread_create(&thread_id[i], NULL, init_arr, &thread_num[i]);
	if(err)
	  {
	...
	for(j = 0; j < height; j++)
	  {
	    printf( "%.0f %.0f %.0f\n",
                    threads[i][j][0], threads[i][j][1], threads[i][j][2] );
	  }
	printf( "\n" );
      }
